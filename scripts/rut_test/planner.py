...
class ConstrainedPlanner(object):
    ...
    def BiRRTPlanner(self, manip, start, goalLocation, goal_type, constraints=None, grasp=None, collision=None):
        ...
        if collision is not None:
            self.collObjName = collision[0]
            self.collRelation = collision[1]
        else:
            self.collObjName = None
            self.collRelation = None
    ...
    def addRootConfiguration(self, T):
        ...
        while searching and (time.time() - self.tstart) < self.TOTAL_TIME:
            ...
            if self.evaluateConstraints(ConstraintType.GOAL_EE, pose=ee_pose):
                config = self.manip.RandomIK(ee_pose, objName=self.collObjName, relation=self.collRelation)
    ...
    def checkEdgeCollision(self, q, q_parent):
        ...
        if not self.manip.IsCollisionFree(q, objName=self.collObjName, relation=self.collRelation):
            return False
        cdist = self.cspaceLength([q_parent, q])
        ...
        return all((self.manip.IsCollisionFree(m, objName=self.collObjName, relation=self.collRelation) for m in middle_qs)) 
