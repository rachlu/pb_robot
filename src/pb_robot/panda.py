...
class Panda(pb_robot.body.Body):
    ...
    def __init__(self):
        ...
        self.arm = Manipulator(self.id, self.arm_joints, self.hand, 'panda_hand', self.ik_info, self.torque_limits,
                               self.startq)
...
class Manipulator(object):
    ...
    def Grab(self, obj, relation, t):
        ...
        @param t Type of object grabbed (M: Mechanism or RB: Rigid Body)'''
        self.grabbedRelations[obj.get_name()] = relation
        self.grabbedObjects[obj.get_name()] = obj
        self.grabbedTypes[obj.get_name()] = t
    ...
    def Release(self, obj):
        ...
        self.grabbedTypes.pop(obj.get_name(), None)
    ...
    def ComputeIKQ(self, transform, seed_q, max_distance=0.3):
        ...
        bias we attempt to find an IK without that bias
        @param transform 4x4 desired pose of end effector
        @param seed_q Configuration to bias the IK
        @return Nx1 configuration if successful, else 'None' '''
        ...
        for i in self.__robot.all_links:
            for j in self.__robot.all_links:
                ...
                if (abs(linkI - linkJ) < 2) or (linkI == 8) or (linkJ == 8):
                    break
                pts = p.getClosestPoints(self.__robot.id, self.__robot.id, distance=0.01, linkIndexA=linkI,
                                         linkIndexB=linkJ)
                if len(pts) > 0:
                    return False
        return True
